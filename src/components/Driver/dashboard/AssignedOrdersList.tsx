...
import { MapPin, Navigation, Package, CheckCircle, Truck, XCircle } from "lucide-react";
import { markDispatchedAction, submitDeliveryProofAction } from "@/app/driver/dashboard/actions";
...
export default function AssignedOrdersList({ orders }: Props) {
  ...
  function openProofModal(order: AssignedOrder) {
    setModal({ orderId: order.id, patientName: order.patient_name });
  }

  async function handleProofSubmit(fd: FormData): Promise<{ error: string | null }> {
    if (!modal) return { error: "No order selected" };
    setModalSubmitting(true);
    markLoading([modal.orderId]);
    const result = await submitDeliveryProofAction(modal.orderId, fd);
    setModalSubmitting(false);
    if (!result.error) {
      setModal(null);
      clearLoading([modal.orderId]);
    } else {
      clearLoading([modal.orderId]);
    }
    return result;
    ...
    <>
      {modal && (
        <DeliveryProofModal
          ...
          onClose={() => !modalSubmitting && setModal(null)}
                  ...
                  <div key={order.id} className="bg-white rounded-2xl border border-teal-200 shadow-sm p-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        {isLoading ? "Updating…" : "Complete Delivery"}
                      </button>
                    </div>

                    {/* Failed delivery shortcut — subtle link below main actions */}
                    <button
                      onClick={() => openProofModal(order)}
                      disabled={pending || isLoading}
                      className="mt-2 w-full text-[11px] text-red-400 flex items-center justify-center gap-1 hover:text-red-600 disabled:opacity-40"
                    >
